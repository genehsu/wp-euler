...
# NOTE: the generator will work for 1.8.7, not 1.8.6
...
def figurate(base, n)
  ...
  raise "illegal base #{base}"
end

generators = []
3.upto(8) do |base|
  generators << Generator.new do |g|
    n = 1
    while true
      g.yield figurate(base, n)
      n += 1
    end
  end
end

choices = generators.collect { |generator|
  values = generator.take_while { |n| n <= 9999 }
  values.select { |n| n >= 1000 }
}

# each of these is a hash from "first two digits" to a number 
@choice_hashes = choices.collect { |choices_array|
  h = {}
  choices_array.each do |choice_number|
    h[choice_number / 100] = (choice_number % 100)
  end
  h
}

def choice_hash_for_base(n)
  return @choice_hashes[n-3]
end

# returns a sum or nil
...
# start with N=8 because it is smaller
sum = search([], [], [], [8,7,6,5,4,3])
p "sum: #{sum}"
